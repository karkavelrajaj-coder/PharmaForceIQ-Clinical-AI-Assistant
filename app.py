...
# ---------------------------------------------------------------------------
# v3.2 FIX (correction layer): uses citation_verify.py's REAL, unmodified
# detection logic (verify_citations()) -- same exact span/surname matching
# as production -- but instead of only appending a small warning badge
# NEXT TO the wrong text (production's annotate_unverified_citations()
# behavior, which leaves e.g. "Lin et al., 2024" fully visible), this
# REPLACES the fabricated attribution with an honest, non-misleading
# marker. Motivation: real testing found a citation with a genuinely
# correct DOI but the WRONG author name attached (real first author is
# "Chien", model wrote "Lin") -- a badge next to a wrong name still shows
# the wrong name prominently. Replacing it removes the false information
# instead of just flagging it. This is new code, not a copy of a
# production file -- citation_verify.py itself is untouched; this only
# changes what WE do with its (unmodified) detection output.
# ---------------------------------------------------------------------------
def correct_unverified_citations(answer_md: str, papers_cited: list) -> tuple:
    stats = citation_verify.verify_citations(answer_md, papers_cited)
    if not stats["unverified_items"]:
        return answer_md, stats

    replacement = (
        '<span title="The original text named a specific author/year here, but that '
        'attribution could not be verified against the retrieved sources and was removed." '
        'style="color:#a82020;background:#ffe6e6;padding:0 4px;border-radius:3px;'
        'font-size:0.78rem;border:1px solid #f5b5b5;">[citation removed \u2014 unverified]</span>'
    )
    ...
    for it in items:
        ...
        count += 1
    stats["citations_corrected"] = count
    return out, stats
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        # -----------------------------------------------------------------
        # v3.2: replaces the wrong attribution instead of just flagging it
        # next to the wrong text (see correct_unverified_citations() above
        # for why -- real testing found a correct DOI attached to a wrong
        # author name, and a badge alone still left the wrong name visible)
        annotated, citation_stats = correct_unverified_citations(
        ...
        # v3.1 FIX: stats dicts only carry "badges_added"/"citations_corrected"
        # keys on the branch where something was actually found -- when
        # there's nothing to flag, the function returns early and neither
        # key is set. Bracket access crashed on exactly this path in real
        # testing -- .get() is the correct fix.
        if citation_stats.get("citations_corrected", 0) or numeric_stats.get("badges_added", 0):
        ...
        with st.expander("Details", expanded=False):
            ...
            st.markdown(
                ...
                f"({citation_stats['unverified']} corrected) | "
