...
from prompts import build_system_prompt  # the REAL production prompt module, copied verbatim
...
# ---------------------------------------------------------------------------
# CONTEXT FORMATTING — matches the GUIDELINE branch of format_paper_context()
# in app/papers/formatter.py exactly (see module docstring for why this
# branch, not the papers/DOI branch, is the correct match for our data).
# ---------------------------------------------------------------------------
def format_paper_context(chunks: list[dict]) -> str:
    if not chunks:
        return "(No relevant clinical guidelines or reference documents found.)"

    lines = ["=== CLINICAL KNOWLEDGE EVIDENCE ===\n"]
    for idx, c in enumerate(chunks, 1):
        topic = c.get("doc_stem") or "Unknown source"
        section = c.get("section_title") or ""
        citation = f"[{idx}] {topic} | {section}"
        if c.get("score") is not None:
            citation += f"  (relevance: {c['score']:.2f})"
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner(f"Searching with {MODEL_CHOICES[selected_model]['label']}..."):
            ...
            paper_context = format_paper_context(chunks)

        answer = st.write_stream(stream_answer(prompt, paper_context, selected_role, selected_type))
